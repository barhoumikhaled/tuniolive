...
export async function GET() {
  ...
  const [arOutstanding] = await db.select({
    total: sql<string>`coalesce(sum(cast(coalesce(${arInvoiceItemsTable.qtyBox},'0') as numeric) * cast(coalesce(${arInvoiceItemsTable.priceBox},'0') as numeric)), 0)::text`
  }).from(arInvoiceItemsTable)
    .innerJoin(arInvoicesTable, eq(arInvoiceItemsTable.invoiceId, arInvoicesTable.id))
    .where(sql`${arInvoicesTable.paymentStatus} != 'Paid'`);
